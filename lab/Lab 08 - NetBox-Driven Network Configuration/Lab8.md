...
Terraform confirms that TCP port 22 accepts a socket, but IOS XE can briefly reset SSH connections before its CLI service is fully initialized. An `Error reading SSH protocol banner` or `Connection reset by peer` immediately after `terraform-dev` does not by itself indicate incorrect credentials. Both Ansible playbooks first use an authenticated `wait_for_connection` check for up to five minutes, retrying every ten seconds, before configuring or verifying the router.
