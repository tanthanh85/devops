...
"${vault_cmd[@]}" kv put secret/network/test/c8000v management_ip="$TEST_ROUTER_IP" prefix_length="$TEST_ROUTER_PREFIX_LENGTH" gateway="$TEST_ROUTER_GATEWAY" username="$TEST_ROUTER_USERNAME" password="$TEST_ROUTER_PASSWORD" port=22 enabled=true >/dev/null
...
echo "Stored CML, ephemeral test-router, and Elastic audit integration records in Vault."
